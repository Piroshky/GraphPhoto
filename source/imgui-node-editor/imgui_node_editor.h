...
struct Object {
  ...
  struct Pin final: Object {
    ...
    Pin(EditorContext* editor, PinId id, PinDirection kind)
      : Object(editor)
      , m_ID(id)
      , m_Kind(kind)
      , m_Node(nullptr)
...
// };
  ...
  // using ax::NodeEditor::PinDirection;;
...
struct FlowAnimationController final : AnimationController {
  FlowAnimationController(EditorContext* e);
...
// @Todo como up with real name for this function if it even gets called anywhere at all
...
struct EditorContext {
  ...
  bool LLink(LinkId id, PinId startPinId, PinId endPinId, const ImVec4& color, float thickness);
  ...
  bool QueryNewNode(PinId* pinId, const ImVec4& color, float thickness);bool AcceptNewItem();
  bool AcceptNewItem(const ImVec4& color, float thickness);
  void RejectNewItem();
  void RejectNewItem(const ImVec4& color, float thickness);
  void EndCreate();
  bool BeginDelete();
  bool QueryDeletedLink(LinkId* linkId, PinId* startId = nullptr, PinId* endId = nullptr);
  bool QueryDeletedNode(NodeId* nodeId);
  ...
  void SelectLink(LinkId linkId, bool append);
  void DeselectNode(NodeId nodeId);
  void DeselectLink(LinkId linkId);
  ...
  bool QueryNewLink(PinId* startId, PinId* endId);
  bool QueryNewLink(PinId* startId, PinId* endId, const ImVec4& color, float thickness);
  ...
  Pin*    CreatePin(PinId id, PinDirection kind);
