...
//------------------------------------------------------------------------------
static bool IsGroup(const Node* node) {
  return (node && node->m_Type == NodeType::Group);
}
...
//
//------------------------------------------------------------------------------
NodeBuilder::NodeBuilder(NodeEditor::EditorContext* editor):
  Editor(editor),
...
static EditorContext *editor = nullptr;
EditorContext* CreateEditor(const Config* config) {
  if(!editor) {
    editor = new EditorContext(config);
    return editor;
  } else {
    return editor;
  }
...
void EditorContext::PinRect(const ImVec2& a, const ImVec2& b) {
  m_NodeBuilder.PinRect(a, b);
}

//@TODO I don't know what these are for, but they never get called
// void EditorContext::PinPivotRect(const ImVec2& a, const ImVec2& b)
// {
//   m_NodeBuilder.PinPivotRect(a, b);
// }

// void EditorContext::PinPivotSize(const ImVec2& size)
// {
//   m_NodeBuilder.PinPivotSize(size);
// }

// void PinPivotScale(const ImVec2& scale)
// {
//   m_NodeBuilder.PinPivotScale(scale);
// }
...
{
  return m_HintBuilder.Begin(nodeId);
}

ImVec2 EditorContext::GetGroupMin()
{
  return m_HintBuilder.GetGroupMin();
}

ImVec2 EditorContext::GetGroupMax()
{
  return m_HintBuilder.GetGroupMax();
}

ImDrawList* EditorContext::GetHintForegroundDrawList()
{
  return m_HintBuilder.GetForegroundDrawList();
}

ImDrawList* EditorContext::GetHintBackgroundDrawList()
{
  return m_HintBuilder.GetBackgroundDrawList();
}
...
//@todo 2021-06-10  bug not in here
ImDrawList* EditorContext::GetNodeBackgroundDrawList(NodeId nodeId) {
  if (auto node = FindNode(nodeId)) {
    return m_NodeBuilder.GetUserBackgroundDrawList(node);
  } else {
    return nullptr;
  }
}

void EditorContext::Flow(LinkId linkId) {
  if (auto link = FindLink(linkId))
    Flow(link);
}
...
bool EditorContext::BeginDelete() {
  // auto& context = GetItemDeleter();
  // return context.Begin();

  return m_DeleteItemsAction.Begin();
}
...
bool EditorContext::AcceptDeletedItem() {
  auto& context = GetItemDeleter();
...
void EditorContext::RestoreNodeState(NodeId nodeId) {
  if (auto node = FindNode(nodeId))
    MarkNodeToRestoreState(node);
}



int EditorContext::GetSelectedObjectCount() {
  return (int)GetSelectedObjects().size();
}

int EditorContext::GetSelectedNodes(NodeId* nodes, int size) {
  return BuildIdList(GetSelectedObjects(), nodes, size, [](auto object)
		     {
...
int EditorContext::GetSelectedLinks(LinkId* links, int size) {
  return BuildIdList(GetSelectedObjects(), links, size, [](auto object)
		     {
...
bool EditorContext::DeleteNode(NodeId nodeId) {
  if (auto node = FindNode(nodeId))
    return GetItemDeleter().Add(node);
  else
    return false;
}
...
void EditorContext::EndShortcut() {
  return GetShortcut().End();
}

float EditorContext::GetCurrentZoom() {
  return GetView().InvScale;
}
...
Node* NodeEditor::EditorContext::GetNode(NodeId id)
{
...
void NodeBuilder::PinRect(const ImVec2& a, const ImVec2& b)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void NodeBuilder::PinPivotSize(const ImVec2& size)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void NodeBuilder::PinPivotScale(const ImVec2& scale)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void NodeBuilder::PinPivotAlignment(const ImVec2& alignment)
{
  IM_ASSERT(nullptr != m_CurrentPin);
...
void NodeBuilder::Group(const ImVec2& size)
{
  IM_ASSERT(nullptr != m_CurrentNode);
  IM_ASSERT(nullptr == m_CurrentPin);
  IM_ASSERT(false   == m_IsGroup);
...
ImDrawList* NodeBuilder::GetUserBackgroundDrawList() const {
...
void EditorContext::Begin(const char* id, const ImVec2& size) {
  ...
  // printf("Editor num nodes: %d\n",m_Nodes.size());
  // printf("Editor num pins: %d\n",m_Pins.size());
  // printf("Editor link pins: %d\n",m_Links.size());
...
bool EditorContext::LLink(LinkId id, PinId startPinId, PinId endPinId, const ImVec4& color, float thickness)
{
  return DoLink(id, startPinId, endPinId, ImColor(color), thickness);
}

bool EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness)
{
  //auto& editorStyle = GetStyle();
...
void EditorContext::MarkNodeToRestoreState(Node* node)
{
  node->m_RestoreState = true;
}

void EditorContext::RestoreNodeState(Node* node)
{
...
void EditorContext::SelectObject(Object* object)
{
  m_SelectedObjects.push_back(object);
}

void EditorContext::DeselectObject(Object* object)
{
...
void EditorContext::SetSelectedObject(Object* object)
{
  ClearSelection();
  SelectObject(object);
}

void EditorContext::ToggleObjectSelection(Object* object)
{
...
bool EditorContext::IsSelected(Object* object)
{
  return std::find(m_SelectedObjects.begin(), m_SelectedObjects.end(), object) != m_SelectedObjects.end();
}

const vector<Object*>& EditorContext::GetSelectedObjects()
{
  return m_SelectedObjects;
}

  bool EditorContext::IsAnyNodeSelected()
  {
...
bool EditorContext::IsAnyLinkSelected()
{
  for (auto object : m_SelectedObjects)
    if (object->AsLink())
      return true;
...
Node* EditorContext::FindNodeAt(const ImVec2& p)
{
  for (auto node : m_Nodes)
    if (node->TestHit(p))
      return node;
...
void EditorContext::FindNodesInRect(const ImRect& r, vector<Node*>& result, bool append, bool includeIntersecting)
{
  if (!append)
    result.resize(0);
...
void EditorContext::FindLinksInRect(const ImRect& r, vector<Link*>& result, bool append)
{
  if (!append)
    result.resize(0);
...
void EditorContext::FindLinksForNode(NodeId nodeId, vector<Link*>& result, bool add)
{
  if (!add)
    result.clear();
...
void EditorContext::NotifyLinkDeleted(Link* link) {
  if (m_LastActiveLink == link)
    m_LastActiveLink = nullptr;
}

void EditorContext::Suspend(SuspendFlags flags)
{
  auto drawList = ImGui::GetWindowDrawList();
...
void EditorContext::Resume(SuspendFlags flags)
{
  auto drawList = ImGui::GetWindowDrawList();
  if ((flags & SuspendFlags::KeepSplitter) != SuspendFlags::KeepSplitter)
    ImDrawList_SwapSplitter(drawList, m_Splitter);
...
Node* EditorContext::CreateNode(NodeId id)
{
  IM_ASSERT(nullptr == FindObject(id));
...
//
//------------------------------------------------------------------------------
void Pin::Draw(ImDrawList* drawList, DrawFlags flags) {
    if (flags & Hovered)
    {
...
ImVec2 Pin::GetClosestPoint(const ImVec2& p) const {
    return ImRect_ClosestPoint(m_Pivot, p, true, m_Radius + m_ArrowSize);
}

ImLine Pin::GetClosestLine(const Pin* pin) const {
...
//
//------------------------------------------------------------------------------
bool Node::AcceptDrag()
{
    m_DragStart = m_Bounds.Min;
    return true;
}

void Node::UpdateDrag(const ImVec2& offset)
{
...
void Node::GetGroupedNodes(std::vector<Node*>& result, bool append)
{
    if (!append)
        result.resize(0);
...
ImRect Node::GetRegionBounds(NodeRegion region) const
{
...
NodeRegion Node::GetRegion(const ImVec2& point) const
{
    if (m_Type == NodeType::Node)
    {
...
//
//------------------------------------------------------------------------------
void Link::Draw(ImDrawList* drawList, DrawFlags flags)
{
    if (flags == None)
    {
...
void Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
    if (!m_IsLive)
        return;
...
ImCubicBezierPoints Link::GetCurve() const
{
...
bool Link::TestHit(const ImVec2& point, float extraThickness) const
{
    if (!m_IsLive)
        return false;
...
bool Link::TestHit(const ImRect& rect, bool allowIntersect) const
{
    if (!m_IsLive)
        return false;
...
Link* EditorContext::CreateLink(LinkId id)
{
    IM_ASSERT(nullptr == FindObject(id));
...
Node* EditorContext::FindNode(NodeId id) {
    return FindItemInLinear(m_Nodes, id);
}

Pin* EditorContext::FindPin(PinId id) {
    return FindItemIn(m_Pins, id);
}

Link* EditorContext::FindLink(LinkId id)
{
    return FindItemIn(m_Links, id);
}

Object* EditorContext::FindObject(ObjectId id)
{
    if (id.IsNodeId())
...
Link* EditorContext::FindLinkAt(const ImVec2& p)
{
    for (auto& link : m_Links)
...
ImU32 EditorContext::GetColor(StyleColor colorIndex) const
{
    return ImColor(m_Style.Colors[colorIndex]);
}

ImU32 EditorContext::GetColor(StyleColor colorIndex, float alpha) const
{
    auto color = m_Style.Colors[colorIndex];
    return ImColor(color.x, color.y, color.z, color.w * alpha);
}

void EditorContext::RegisterAnimation(Animation* animation)
{
    m_LiveAnimations.push_back(animation);
}

void EditorContext::UnregisterAnimation(Animation* animation)
{
...
void EditorContext::UpdateAnimations()
{
...
void EditorContext::SetUserContext(bool globalSpace)
{
...
Control EditorContext::BuildControl(bool allowOffscreen) {
...
void EditorContext::ShowMetrics(const Control& control)
{
    auto& io = ImGui::GetIO();
...
//
//------------------------------------------------------------------------------
void NodeSettings::ClearDirty()
{
    m_IsDirty     = false;
    m_DirtyReason = SaveReasonFlags::None;
}

void NodeSettings::MakeDirty(SaveReasonFlags reason)
{
    m_IsDirty     = true;
    m_DirtyReason = m_DirtyReason | reason;
}

json::value NodeSettings::Serialize()
{
    json::value result;
...
bool NodeSettings::Parse(const std::string& string, NodeSettings& settings)
{
...
bool NodeSettings::Parse(const json::value& data, NodeSettings& result)
{
    if (!data.is_object())
        return false;
...
//
//------------------------------------------------------------------------------
NodeSettings* Settings::AddNode(NodeId id)
{
    m_Nodes.push_back(NodeSettings(id));
    return &m_Nodes.back();
}

NodeSettings* Settings::FindNode(NodeId id)
{
...
void Settings::ClearDirty(Node* node)
{
    if (node)
    {
...
void Settings::MakeDirty(SaveReasonFlags reason, Node* node)
{
    m_IsDirty     = true;
    m_DirtyReason = m_DirtyReason | reason;
...
std::string Settings::Serialize()
{
    ...
    auto serializeObjectId = [](ObjectId id)
    {
        ...
        {
            default:
            case ObjectType::NONE: return value;
            case ObjectType::NODE: return "node:" + value;
            case ObjectType::LINK: return "link:" + value;
            case ObjectType::PIN:  return "pin:"  + value;
        }
    };
...
bool Settings::Parse(const std::string& string, Settings& settings)
{
...
//
//------------------------------------------------------------------------------
Animation::Animation(EditorContext* editor):
    Editor(editor),
    m_State(Stopped),
    m_Time(0.0f),
    m_Duration(0.0f)
...
void Animation::Play(float duration)
{
    if (IsPlaying())
        Stop();
...
void NavigateAnimation::NavigateTo(const ImRect& target, float duration)
{
    Stop();
...
void NavigateAnimation::OnUpdate(float progress)
{
    ImRect current;
...
void NavigateAnimation::OnStop()
{
    Editor->MakeDirty(SaveReasonFlags::Navigation);
}

void NavigateAnimation::OnFinish()
{
...
//
//------------------------------------------------------------------------------
FlowAnimation::FlowAnimation(FlowAnimationController* controller):
...
void FlowAnimation::Flow(Link* link, float markerDistance, float speed, float duration)
{
    Stop();
...
void FlowAnimation::Draw(ImDrawList* drawList)
{
...
void FlowAnimation::ClearPath()
{
    vector<CurvePoint>().swap(m_Path);
    m_PathLength = 0.0f;
}

ImVec2 FlowAnimation::SamplePath(float distance)
{
...
void FlowAnimation::OnUpdate(float progress)
{
    IM_UNUSED(progress);
...
//
//------------------------------------------------------------------------------
FlowAnimationController::FlowAnimationController(EditorContext* e):
  editor(e),
...
void FlowAnimationController::Flow(Link* link) {
...
void FlowAnimationController::Draw(ImDrawList* drawList)
{
...
FlowAnimation* FlowAnimationController::GetOrCreate(Link* link)
{
...
void FlowAnimationController::Release(FlowAnimation* animation)
{
    IM_UNUSED(animation);
}
...
//
//------------------------------------------------------------------------------
const float NavigateAction::s_ZoomLevels[] =
{
    0.1f, 0.15f, 0.20f, 0.25f, 0.33f, 0.5f, 0.75f, 1.0f, 1.25f, 1.50f, 2.0f, 2.5f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f
};

const int NavigateAction::s_ZoomLevelCount = sizeof(s_ZoomLevels) / sizeof(*s_ZoomLevels);

NavigateAction::NavigateAction(EditorContext* e, ImGuiEx::Canvas& canvas):
  editor(e),
  EditorAction(e),
  m_IsActive(false),
  m_Zoom(1),
...
bool NavigateAction::HandleZoom(const Control& control)
{
    IM_UNUSED(control);
...
void NavigateAction::ShowMetrics()
{
...
void NavigateAction::NavigateTo(const ImRect& bounds, bool zoomIn, float duration, NavigationReason reason)
{
    if (ImRect_IsEmpty(bounds))
        return;

    if (duration < 0.0f)
        duration = editor->GetStyle().ScrollDuration;
...
void NavigateAction::NavigateTo(const ImRect& target, float duration, NavigationReason reason)
{
    m_Reason = reason;
...
void NavigateAction::StopMoveOverEdge()
{
    if (m_MovingOverEdge)
    {
        Editor->MakeDirty(SaveReasonFlags::Navigation);
...
void NavigateAction::SetWindow(ImVec2 position, ImVec2 size)
{
    m_WindowScreenPos  = position;
    m_WindowScreenSize = size;
}

ImGuiEx::CanvasView NavigateAction::GetView() const
{
    return ImGuiEx::CanvasView(-m_Scroll, m_Zoom);
}

ImVec2 NavigateAction::GetViewOrigin() const
{
    return -m_Scroll;
}

float NavigateAction::GetViewScale() const
{
    return m_Zoom;
}

void NavigateAction::SetViewRect(const ImRect& rect)
{
...
float NavigateAction::MatchZoom(int steps, float fallbackZoom)
{
...
int NavigateAction::MatchZoomIndex(int direction)
{
    int   bestIndex    = -1;
...
//
//------------------------------------------------------------------------------
SizeAction::SizeAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_Clean(false),
    m_SizedNode(nullptr),
...
AcceptResult SizeAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool SizeAction::Process(const Control& control)
{
    if (m_Clean)
    {
        m_Clean = false;
...
NodeRegion SizeAction::GetRegion(Node* node)
{
    return node->GetRegion(ImGui::GetMousePos());
}

ImGuiMouseCursor SizeAction::ChooseCursor(NodeRegion region)
{
    switch (region)
    {
        default:
...
//
//------------------------------------------------------------------------------
DragAction::DragAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_Clear(false),
...
AcceptResult DragAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool DragAction::Process(const Control& control) {
  if (m_Clear) {
    m_Clear = false;
...
//
//------------------------------------------------------------------------------
SelectAction::SelectAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
...
AcceptResult SelectAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool SelectAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void SelectAction::Draw(ImDrawList* drawList)
{
...
//
//------------------------------------------------------------------------------
ContextMenuAction::ContextMenuAction(EditorContext* editor):
    EditorAction(editor),
...
AcceptResult ContextMenuAction::Accept(const Control& control)
{
...
bool ContextMenuAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void ContextMenuAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
bool ContextMenuAction::ShowNodeContextMenu(NodeId* nodeId)
{
...
bool ContextMenuAction::ShowPinContextMenu(PinId* pinId)
{
...
bool ContextMenuAction::ShowLinkContextMenu(LinkId* linkId)
{
...
bool ContextMenuAction::ShowBackgroundContextMenu()
{
...
//
//------------------------------------------------------------------------------
ShortcutAction::ShortcutAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_InAction(false),
...
AcceptResult ShortcutAction::Accept(const Control& control)
{
...
bool ShortcutAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void ShortcutAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
void ShortcutAction::End()
{
    if (m_IsActive)
        m_InAction = false;
}

bool ShortcutAction::AcceptCut()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Cut;
}

bool ShortcutAction::AcceptCopy()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Copy;
}

bool ShortcutAction::AcceptPaste()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Paste;
}

bool ShortcutAction::AcceptDuplicate()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Duplicate;
}

bool ShortcutAction::AcceptCreateNode()
{
    IM_ASSERT(m_InAction);
...
//
//------------------------------------------------------------------------------
CreateItemAction::CreateItemAction(EditorContext* editor):
    EditorAction(editor),
    m_InActive(false),
    m_NextStage(None),
    m_CurrentStage(None),
...
bool CreateItemAction::Process(const Control& control) {
    IM_ASSERT(m_IsActive);
    ...
    if (m_DraggedPin && control.ActivePin == m_DraggedPin && (m_CurrentStage == Possible)) {
        const auto draggingFromSource = (m_DraggedPin->m_Kind == PinDirection::OUTPUT);

        Pin cursorPin(Editor, 0, draggingFromSource ? PinDirection::INPUT : PinDirection::OUTPUT);
        ...
        NodeEditor::Link candidate(Editor, 0);
...
void CreateItemAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
void CreateItemAction::SetStyle(ImU32 color, float thickness)
{
    m_LinkColor     = color;
...
void CreateItemAction::DropNode()
{
    IM_ASSERT(!m_InActive);
...
void CreateItemAction::DropNothing()
{
    IM_ASSERT(!m_InActive);
...
CreateItemAction::Result CreateItemAction::RejectItem()
{
    IM_ASSERT(m_InActive);
...
CreateItemAction::Result CreateItemAction::QueryLink(PinId* startId, PinId* endId) {
    IM_ASSERT(m_InActive);
...
CreateItemAction::Result CreateItemAction::QueryNode(PinId* pinId)
{
    IM_ASSERT(m_InActive);
...
//
//------------------------------------------------------------------------------
DeleteItemsAction::DeleteItemsAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
...
AcceptResult DeleteItemsAction::Accept(const Control& control) {
    IM_ASSERT(!m_IsActive);
    ...
    auto addDeadLinks = [this]() {
      std::vector<NodeEditor::Link*> links;
      for (auto object : m_CandidateObjects) {
	auto node = object->AsNode();
	if (!node)
	  continue;
...
bool DeleteItemsAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void DeleteItemsAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
bool DeleteItemsAction::Add(Object* object)
{
...
// just finds out if the item exists
bool DeleteItemsAction::QueryLink(LinkId* linkId, PinId* startId, PinId* endId) {
    ObjectId objectId;
...
bool DeleteItemsAction::QueryNode(NodeId* nodeId)
{
    ObjectId objectId;
...
bool DeleteItemsAction::QueryItem(ObjectId* itemId, IteratorType itemType)
{
    if (!m_InInteraction)
        return false;
...
bool DeleteItemsAction::AcceptItem() {
    if (!m_InInteraction)
        return false;
...
void DeleteItemsAction::RejectItem()
{
    if (!m_InInteraction)
        return;
...
void DeleteItemsAction::RemoveItem() {
    auto item = m_CandidateObjects[m_CandidateItemIndex];
...
//
//------------------------------------------------------------------------------
HintBuilder::HintBuilder(EditorContext* editor):
    Editor(editor),
    m_IsActive(false),
    m_CurrentNode(nullptr)
{
...
bool HintBuilder::Begin(NodeId nodeId)
{
    IM_ASSERT(nullptr == m_CurrentNode);
...
ImVec2 HintBuilder::GetGroupMin()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImVec2 HintBuilder::GetGroupMax()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImDrawList* HintBuilder::GetForegroundDrawList()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImDrawList* HintBuilder::GetBackgroundDrawList()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
//
//------------------------------------------------------------------------------
void Style::PushColor(StyleColor colorIndex, const ImVec4& color)
{
...
const char* Style::GetColorName(StyleColor colorIndex) const
{
    switch (colorIndex)
    {
...
ImVec2* Style::GetVarVec2Addr(StyleVar idx)
{
    switch (idx)
    {
...
ImVec4* Style::GetVarVec4Addr(StyleVar idx)
{
    switch (idx)
    {
...
//
//------------------------------------------------------------------------------
// @todo is this some duplication bs? 
Config::Config(const Config* config)
{
    if (config)
        *static_cast<Config*>(this) = *config;
}
...
std::string Config::LoadNode(NodeId nodeId)
{
    std::string data;
...
void Config::BeginSave()
{
    if (BeginSaveSession)
        BeginSaveSession(UserPointer);
}

bool Config::Save(const std::string& data, SaveReasonFlags flags)
{
    if (SaveSettings)
    {
...
bool Config::SaveNode(NodeId nodeId, const std::string& data, SaveReasonFlags flags)
{
    if (SaveNodeSettings)
